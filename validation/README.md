...
| `ycsb_crash_recover_sweep.sh` with `WL=ycsbw` / `WL=ycsbx` | Deletes under recovery (design, deletes subsection) | The delete workloads (sliding-window churn; the adversarial ycsbx mix that reads, writes, deletes and re-inserts the same keys inside one epoch and across epochs) run with the cache capped (`CAP`, default 1.5 M records) so eviction and the reclaim-first pass execute, with the serial-order oracle on (`EPIC_TIMELINE_ORACLE=1`, see below), and with the drop-one-delete negative control. `NREC` / `NSTART` set the record count and the initial live set. Extra gates: `[STATE-HASH-LIVE]`, `[LIVE-CHECK]` (must print PASS), `[TIMELINE-ORACLE]`, `[MAP-CHECK]`, `[DEAD-CHECK]`; the worker must die at the injected fault and the recovering process must exit clean. |
| `delete_semantics_check.sh` | Deletes and inserts are resolved in serial order (design, deletes subsection) | Runs ycsbx with the host oracle: every operation's resolved record must equal a serial-order replay of the epoch, the GPU index must equal the model's live set at the end (`[MAP-CHECK]`), and no deleted record may be written after its delete (`[DEAD-CHECK]`); both index paths must have run. Runs it again on a map at high occupancy (1.8 M of 3 M records live), where probe chains cross erased slots. Then proves the gate is sensitive: `EPIC_TIMELINE_ORDER_BLIND=1` resolves the way epoch-boundary deletes would on both paths, and the oracle must fail. Then runs ycsbw under the oracle, and checks that `EPIC_TIMELINE_FORCE_GENERAL=1` reproduces the fast path's hashes exactly on ycsbf, ycsbi and ycsbw. Every run must exit clean. |
...
- `EPIC_TIMELINE_ORDER_BLIND=1`: both index paths resolve operations without regard to their serial position (a control; the oracle must fail).
