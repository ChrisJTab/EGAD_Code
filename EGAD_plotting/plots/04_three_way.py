#!/usr/bin/env python3
"""Paper figure 04: staging overhead when staging isn't strictly needed.

Renders a 2x2 grid (one panel per YCSB workload) of skew sweeps comparing
hybrid_staging against gpu_only when the database fits in HBM (5 M records
at 120 B, -rT-fF). cpu_only is intentionally absent: at this DB size, no
one would pick cpu_only over gpu_only, so showing it would be misleading.

The story this plot tells is "the staging machinery's per-epoch cost when
no eviction is happening" - the gap between solid (hybrid) and dashed
(gpu_only) is the overhead the staging design pays in the fits-in-HBM
regime.

Reads $EGAD_LOGS_DIR/three_way/*.log; writes
$EGAD_FIGURES_DIR/three_way.{pdf,png,csv}.
"""
...
LOG_DIR = os.path.join(LOGS_DIR, "three_way")
OUT_BASE = os.path.join(FIGURES_DIR, "three_way")
...
# 120 B = blue family. Solid = hybrid, dashed = gpu_only.
C_HYBRID = "#1f4e79"
C_GPU    = "#5b9bd5"
...
def render(cells: Dict[Tuple[str, str, float], List[float]]) -> None:
    ...
    if not skews:
        raise SystemExit("no cells parsed; did 04_three_way.sh run?")

    fig, axes = plt.subplots(1, 4, figsize=(14, 3.0), sharex=True, sharey=True)

    series_cfg = [
        ("gpu_only",       C_GPU,    "--", "x", "EPIC-GPU"),
        ("hybrid_staging", C_HYBRID, "-",  "o", "EGAD"),
    ]

    for ax, bench in zip(axes, WORKLOAD_ORDER):
        for mode, color, ls, marker, label in series_cfg:
    ...
    # fig.suptitle(
    #     "YCSB -rT-fF, 120 B, 5 M records (DB fits HBM): staging overhead vs gpu_only ceiling",
    #     fontsize=10, y=1.06,
    # )
    fig.tight_layout()
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["bench", "mode", "skew", "median_mtxn_s", "n_reps"])
        for (bench, mode, skew), vs in sorted(cells.items()):
            w.writerow([bench, mode, skew, statistics.median(vs), len(vs)])
...
def main() -> None:
    if not os.path.isdir(LOG_DIR):
        raise SystemExit(
            f"log dir not found: {LOG_DIR}; run 04_three_way.sh first"
        )
    cells = collect()
    render(cells)
