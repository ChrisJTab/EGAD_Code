...
{
    // cudaDeviceScheduleYield: sched_yield() loop on the host instead of
    // parking on a driver IRQ. Accepted after context creation.
    cudaError_t rc = cudaSetDeviceFlags(cudaDeviceScheduleYield);
    if (rc != cudaSuccess) {
        Logger::GetInstance().Warn(
            "setDeviceScheduleYield: cudaSetDeviceFlags failed: {}",
