    ...
    if (std::getenv("EPIC_TPCC_SEED")) {
        ...
        if (config.execution_mode == ExecMode::HYBRID_STAGING
            ...
            if (auto* gi = dynamic_cast<TpccGpuIndex<TpccTxnArrayT, TpccTxnParamArrayT>*>(index.get())) {
                ...
                // mapping the logs imply, and every delivered key must be
                // absent (a delivered key is never re-inserted).
                {
                    ...
                    dead.reserve(del);
                    for (uint32_t j = 0; j < del; ++j) dead.push_back(cpu_shadow_->durableNoDeleteKey(j));
