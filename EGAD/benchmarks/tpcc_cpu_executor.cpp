    ...
    DeliveryTxnParams *txn, DeliveryTxnExecPlan *plan, uint32_t epoch)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        // Per-district deliver skip, mirroring the GPU executor's sentinel
        // handling: a district with nothing to deliver carries the pipeline
        // invalid-CRID sentinel in every id slot and num_items[i] == 0. The
        // stock loop dereferenced the sentinel as a record id, a wild read
        // far past the table (upstream leaves the id uninitialized after a
        // Release-build no-op assert on the index miss, same failure class).
        if (txn->new_order_id[i] == 0xffffffffu)
        {
            continue;
        }
        NewOrderValue nov;
