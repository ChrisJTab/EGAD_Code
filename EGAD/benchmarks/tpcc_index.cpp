...
void TpccCpuIndex<TxnArrayType, TxnParamArrayType>::indexTxnReads(NewOrderTxnInput<FixedSizeTxn> *txn, void *index_ptr, uint32_t epoch_id)
{
    ...
    index->c_id = txn->c_id; // deterministic logical c_id for the order record's o_c_id
    ...
    for (uint32_t i = 0; i < txn->num_items; ++i)
    {
        ...
        index->items[i].i_id = txn->items[i].i_id; // deterministic logical item id for ol_i_id
        index->items[i].supply_w_id = txn->items[i].w_id; // deterministic supply w_id for ol_supply_w_id
    }
}
