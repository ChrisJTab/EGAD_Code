...
struct NewOrderTxnParams
{
    struct ItemParams
    {
        ...
        uint32_t order_quantities;
        uint32_t i_id;        /* logical item id, carried for the order_line ol_i_id
                                 (item_id becomes the non-deterministic GPU seat post-remap) */
        uint32_t supply_w_id; /* logical supply warehouse id, carried for the order_line
                                 ol_supply_w_id (the prior params->warehouse_id is the
                                 warehouse GPU seat, non-deterministic once W is large
                                 enough that the warehouse cache is not identity-mapped) */
    };
    ...
    // Logical customer c_id (1..3000), carried verbatim from the txn input so the
    // executor can write a DETERMINISTIC o_c_id into the order/new_order record.
    // It is plain data, not a record id, so the CRID->GRID remap leaves it alone
    // (using customer_id here would write the non-deterministic GPU cache seat).
    uint32_t c_id;
    bool all_local;
...
struct DeliveryTxnInput
{
    ...
    uint32_t carrier_id;
    uint32_t delivery_d;
    uint32_t o_id[10]; // per-district oldest-undelivered order id (real TPC-C delivers each district's own order); 0 means nothing to deliver in that district
