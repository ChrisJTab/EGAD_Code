...
struct InsertFullPlan {
    uint32_t write_locs[10];
    uint32_t field_grids[10];  // NEW: per-field GRIDs in split mode
};
...
struct YcsbExecPlan
{
    union Plan
    {
        ...
        struct
        {
            ...
            uint32_t field_grids[10];  // per-field GRIDs in split+hybrid mode (logical field j -> GRID)
        } full_read_plan;
        struct
        {
            uint32_t write_loc;
        ...
        struct
        {
            uint32_t read_locs[10];
            uint32_t write_loc;
            uint32_t field_grids[10];  // per-field GRIDs in split+hybrid mode
        ...
        //     uint32_t write_locs[10];
        // } insert_full_plan;
        InsertFullPlan insert_full_plan;
    } plans[10];
};

// Union size = full_read_modify_write_plan (40 read_locs + 4 write_loc + 40 field_grids = 84 bytes).
// With 10 plans per txn: 10 * 84 = 840.
static_assert(sizeof(YcsbExecPlan) == 840);
