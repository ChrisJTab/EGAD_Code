...
void TpccTxnGenerator::generateTxn(DeliveryTxnInput *txn, uint32_t timestamp)
{
    ...
    // Use the deterministic logical timestamp (epoch*num_txns + i), not the wall
    // clock. delivery_d lands in order_line.ol_delivery_d, so time(nullptr) made
    // that field differ every run and broke deterministic replay; the value is an
    // immaterial date placeholder that is never read back for results.
    txn->delivery_d = timestamp;
    // Per-district deliver: each district delivers its own oldest undelivered
    // order. nextDeliverOId returns 0 (sentinel) for any district with nothing
    // to deliver; the GPU index/aux-index skip those districts.
    for (int d = 0; d < 10; ++d)
    {
        txn->o_id[d] = o_id_gen.nextDeliverOId(txn->w_id - 1, d);
    }
}
