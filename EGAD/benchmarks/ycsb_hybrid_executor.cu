...
__global__ void gpuPiecewiseExecKernel(YcsbConfig config, void *records, void *versions, GpuTxnArray txns,
    ...
    uint8_t *dirty_v1, uint8_t *dirty_v2)
{
    constexpr uint32_t leader_lane = 0;
    constexpr uint32_t all_lanes_mask = 0xffffffffu;
    ...
    case YcsbOpType::FULL_READ: {
        // Per-field GRID from the plan (populated by fill_full_read_field_grids during
        // prepareEpoch). Lanes 10..31 are inactive via active_mask=0x3ffu; clamp the
        // index so all lanes load from a valid slot in the plan struct.
        uint32_t field_id = plan->plans[warp_piece_id].full_read_plan.field_grids[min(lane_id, 9u)];
    ...
    case YcsbOpType::FULL_READ_MODIFY_WRITE: {
        // Per-field GRIDs from the plan. Read all 10 fields, then write one specific field.
        uint32_t field_id = plan->plans[warp_piece_id].full_read_modify_write_plan.field_grids[min(lane_id, 9u)];
        gpuReadMultipleFromTableCoop(reinterpret_cast<YcsbFieldRecords *>(records),
            reinterpret_cast<YcsbFieldVersions *>(versions), field_id,
            plan->plans[warp_piece_id].full_read_modify_write_plan.read_locs[min(lane_id, 9u)], epoch, data, lane_id,
            0x3ffu);
        field_id = plan->plans[warp_piece_id].full_read_modify_write_plan.field_grids[txn->field_ids[warp_piece_id]];
        gpuWriteToTableCoop(reinterpret_cast<YcsbFieldRecords *>(records),
            reinterpret_cast<YcsbFieldVersions *>(versions), field_id,
    ...
    case YcsbOpType::INSERT: {
        for (int i = 0; i < 10; ++i)
        {
            uint32_t grid = plan->plans[warp_piece_id].insert_full_plan.field_grids[i];

            gpuWriteToTableCoop(reinterpret_cast<YcsbFieldRecords *>(records),
                reinterpret_cast<YcsbFieldVersions *>(versions), grid,
...
void HybridExecutor::execute(uint32_t epoch)
{
    ...
    if (config.split_field)
    {
        gpuPiecewiseExecKernel<<<num_blocks, block_size>>>(config,
            ...
            config.num_txns, epoch, dirty_v1_, dirty_v2_);
