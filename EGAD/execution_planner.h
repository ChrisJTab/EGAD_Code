...
constexpr op_t r_w_mask = 0x0000000000000800ull;
constexpr op_t r_w_shift = 11;
constexpr op_t offset_mask = 0x00000000000007FFull;
...
public:
    ...
    uint32_t curr_num_read_ops = 0;   // NEW
    uint32_t curr_num_write_ops = 0;  // NEW
    uint32_t curr_num_insert_ops = 0; // count of non-sentinel entries in d_insert_keys

    uint32_t * d_all_keys = nullptr;
    uint32_t * d_read_keys = nullptr;
    uint32_t * d_write_keys = nullptr;
    // Parallel to d_all_keys, sentinel-padded. insert_keys[i] holds the CRID
    // when ops[i] is an INSERT op, otherwise the sentinel. The hybrid stager
    // dedupes alongside d_all_keys to flag insert CRIDs in the working set,
    // so it can split admit-only vs skip-admit during admission.
    uint32_t * d_insert_keys = nullptr;
    // Parallel to d_submitted_ops. d_op_is_insert[op_idx] = 1 iff that op was
    // emitted from a YcsbOpType::INSERT (always also a write_op). The post-
    // submit derive pass turns this into d_insert_keys.
