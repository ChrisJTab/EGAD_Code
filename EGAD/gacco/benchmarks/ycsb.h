...
public:
    ...
    // CPU-side durable shadow of the key->CRID mapping. Owned here so
    // it survives any GPU teardown (parity with epic::ycsb::YcsbBenchmark;
    // gacco does not use the recovery driver itself but YcsbGpuIndex now
    // requires a bound shadow).
    std::unique_ptr<epic::ycsb::YcsbCpuShadowIndex> cpu_shadow_;
