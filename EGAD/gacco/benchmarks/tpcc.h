...
public:
    ...
    // CPU-side durable shadow of the key->CRID mapping. Owned here so it
    // survives any GPU teardown (parity with epic::tpcc::TpccDb; gacco
    // does not use the recovery driver itself but TpccGpuIndex now
    // requires a bound shadow).
    std::unique_ptr<epic::tpcc::TpccCpuShadowIndex> cpu_shadow_;
