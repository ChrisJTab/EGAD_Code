    ...
    , builder(config.num_txns)
{
    ...
    // Construct the CPU shadow first; TpccGpuIndex binds a reference to it.
    const uint32_t maxO_ol = epic::tpcc::flatOLEnabled(config.execution_mode) ? epic::tpcc::computeOLMaxO(config) : 3000u;
    cpu_shadow_ = std::make_unique<epic::tpcc::TpccCpuShadowIndex>(config, maxO_ol);
    index = std::make_shared<TpccGpuIndex<epic::tpcc::TpccTxnArrayT, epic::tpcc::TpccTxnParamArrayT>>(config, *cpu_shadow_);
...
{
    // Parity with epic::tpcc::TpccDb: load the CPU shadow before the GPU
    // index.
