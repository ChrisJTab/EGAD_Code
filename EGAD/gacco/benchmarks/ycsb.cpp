    ...
    , initialization_input(config.num_txns, config.initialize_device, false)
{
    ...
    // Construct the CPU shadow first; YcsbGpuIndex binds a reference to it.
    cpu_shadow_ = std::make_unique<epic::ycsb::YcsbCpuShadowIndex>(config);
...
{
    // Parity with epic::ycsb::YcsbBenchmark: load the CPU shadow before
    // the GPU index so the post-load smoke check sees it populated.
